...
const getAmazonASIN = (url: string) => {
  const fullASINRegex =
    /(?:dp|o|gp|-|gp\/product|\/ASIN|gp\/offer-listing|gp\/product\/images|gp\/aw\/d)\/(B[0-9]{2}[0-9A-Z]{7}|[0-9]{9}(?:X|[0-9]))/g;
  const fullASINSearch = url.match(fullASINRegex);
  if (
    (fullASINSearch && fullASINSearch.length > 0) ||
    url.startsWith("https://amzn.") ||
    url.startsWith("http://amzn.") ||
    url.startsWith("https://www.amzn.") ||
    url.startsWith("http://www.amzn.")
  ) {
